...
:Synopsis: Routine functions for tricky business like Markov dynamics,
diffusion kernels, modularity computation, etc.
...
def compute_CTMC_flow(adj_mat, time):
    ...
    a given graph, over a set of time instants
    ...
    Returns
    -------
    3D array of shape (n_nodes, n_nodes, len(time)), corresponding to the
    flow (one per time point) orderly packed together into an film
...
def sparse_expm_dominant_eigenmodes(A, k=6, t=1.):
    [d, U] = scipy.sparse.linalg.eigs(A, k=k, which='LR')
    n = len(A)
    expo = np.zeros((n, n))
    for j in xrange(U.shape[1]):
        expo += np.exp(t * d[j]) * np.dot(U[:, j:j + 1], U[:, j:j + 1].T)

    return expo


def compute_CTMC_flow_in_logspace(adj_mat, time_resolution=.1,
                                  n_timepoints=10):
    """Computes the CMTC (Continuous-Time Markov Chain) in logspace (base 2)
    time, associated witha a given graph.
    ...
    time_resolution: float (optional, default .1)
        we'll only consider mutliples/bins of this time

    n_timepoints: int (optional, default 10)
        number of time points

    Returns
    -------
    3D array or shape (n_nodes, n_nodes, n_timepoints), corresponding to the
    flow (one per time point) orderly packed together into an film
